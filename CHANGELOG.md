...
1. **默认爬取所有模式**
   - 添加了 `'all'` 作为 `--mode` 参数的选项，默认值设为 `'all'`
   - 重构了主函数，支持先爬取图片模式再爬取小说模式
   - 更新了配置文件默认值为 `'all'`
   - GitHub Actions 工作流默认使用 `'all'` 模式

2. **GitHub Release 功能**
   - 添加了自动创建 GitHub Release 的功能
   - 压缩采集结果为 tar.gz 文件
   - 上传压缩文件作为 Release 资产
   - 包含详细的 Release 描述

3. **远程仓库推送**
   - 实现了 `GitManager` 工具类，用于管理 Git 操作
   - 支持将采集结果推送至远程仓库
   - 支持配置远程仓库 URL、分支、用户名和邮箱
   - 可通过 `config.yaml` 启用或禁用该功能
...
1. **工作流语法错误**
   - 修复了定时任务中 `inputs` 变量不可用的问题
   - 添加了默认值处理，确保工作流在所有情况下都能正常运行

2. **GitHub Actions 版本**
   - 替换了已弃用的 `actions/create-release@v1` 和 `actions/upload-release-asset@v1`
   - 使用最新的 `softprops/action-gh-release@v2` 替代

### 技术栈更新

- 添加了 Git 操作支持
- 增强了 GitHub Actions 工作流
